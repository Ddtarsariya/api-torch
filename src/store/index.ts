...
interface AppState {
  ...
  createEnvironment: (name: string, variables?: KeyValuePair[]) => string;
...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      deleteWorkspace: (id: string) => {
        set((state) => {
          ...
          return {
            ...
            activeWorkspaceId:
              newWorkspaces.length > 0 ? newWorkspaces[0].id : uuidv4(),
          };
        });
      },
          ...
          const updateCollectionItems = (
            ...
            return collections.map((collection) => {
              ...
              if ("items" in collection) {
                return {
                  ...collection,
                  items: updateCollectionItems(
                    collection.items.filter(
                      (item: any) => "items" in item,
                    ) as Collection[],
                  ),
                };
              }
      ...
      createEnvironment: (name: string, variables: KeyValuePair[] = []) => {
      ...
      setActiveEnvironment: (id: string | null) => {
        set((state) => {
          ...
          return {
            workspaces: state.workspaces.map((w) =>
              w.id === workspace.id ? { ...w, activeEnvironmentId: id } : w,
            ),
          };
        });
      },
