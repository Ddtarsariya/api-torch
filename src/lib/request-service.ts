...
import type { ApiRequest, ApiResponse, Environment, KeyValuePair } from '../types';
import { resolveVariables, getContentTypeHeader } from './utils';

// Function to prepare headers with enhanced variable support
function prepareHeaders(headers: KeyValuePair[], environment?: Environment | null): Record<string, string> {
...
function prepareUrl(url: string, params: KeyValuePair[], environment?: Environment | null): string {
  if (!url) return '';
...
function prepareBody(request: ApiRequest, environment?: Environment | null): any {
...
function executePreRequestScript(script: string, request: ApiRequest, environment?: Environment | null): ApiRequest {
  ...
  try {
    ...
    const sandbox = {
      ...
      crypto: {
        getRandomValues: (arr: Uint8Array) => window.crypto.getRandomValues(arr),
    ...
    const scriptFn = new Function('pm', `
      with (pm) {
        ${script}
      }
      return pm.request;
...
function executeTestScript(script: string, response: ApiResponse, environment?: Environment | null): any[] {
  ...
  try {
    ...
    const sandbox = {
      response: {
        ...
        json: () => typeof response.data === 'object' ? response.data : JSON.parse(response.data as string),
      ...
      test: (name: string, fn: () => void) => {
        try {
          ...
          testResults.push({ name, passed: false, message: (error as Error).message });
        }
      },
      expect: (actual: any) => ({
        to: {
    ...
    return [{ name: 'Script execution error', passed: false, message: (error as Error).message }];
  }
}
...
export async function sendRequest(
...
): Promise<{ response: ApiResponse, testResults: any[], metrics?: RequestMetrics }> {
  const startTime = Date.now();
  let metrics: RequestMetrics = { total: 0 };
  ...
  try {
    ...
    return { 
      ...
      testResults: [{ name: 'Request failed', passed: false, message: (error as Error).message }],
      metrics
    };
  }
}
...
export function generateCurlCommand(request: ApiRequest, environment?: Environment | null): string {
  ...
  if (request.body.contentType !== 'none') {
    if (request.body.contentType === 'json') {
      try {
        const jsonBody = typeof request.body.content === 'string' 
