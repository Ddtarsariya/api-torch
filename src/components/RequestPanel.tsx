...
import {
  Play, Plus, Save, Copy, Clock,
  Code, FileJson, List, Server,
  CheckCircle2, AlertCircle, Loader2,
  Sparkles, RotateCw, Bookmark, BookmarkCheck,
...
export const RequestPanel: React.FC = () => {
  ...
  const headerSuggestions = {
    keys: [
      'Accept', 'Authorization', 'Content-Type', 'User-Agent', 'Cache-Control',
      'X-API-Key', 'X-Requested-With', 'Origin', 'Referer', 'Accept-Language'
    ],
    values: [
      'application/json', 'application/xml', 'text/plain', 'text/html',
      'Bearer ', 'Basic ', 'no-cache', 'XMLHttpRequest'
    ]
  };
...
pm.test("Extract auth token", function() {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center w-full">
        ...
        <p className="text-muted-foreground max-w-md mb-6">
          Select a request from the sidebar or create a new one to get started with testing your APIs.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-lg">
          <div className="bg-card border rounded-lg p-4 text-left hover:border-primary/50 hover:shadow-md transition-all cursor-pointer">
            ...
            <p className="text-sm text-muted-foreground">Create a new API request from scratch</p>
          </div>
          <div className="bg-card border rounded-lg p-4 text-left hover:border-primary/50 hover:shadow-md transition-all cursor-pointer">
            <div className="flex items-center text-primary mb-2">
            ...
            <p className="text-sm text-muted-foreground">Import requests from Postman or other tools</p>
          </div>
        </div>
      </div>
    );
  ...
  const handleSendRequest = async () => {
    ...
    setIsLoading(true);
    try {
      const { response, testResults } = await sendRequest(activeRequest, activeEnvironment);
