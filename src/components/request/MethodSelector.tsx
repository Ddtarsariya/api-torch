...
export const MethodBadge: React.FC<{
  ...
  const getMethodColor = () => {
    switch (method) {
      ...
      case "HEAD":
        return "bg-slate-100 text-slate-700 dark:bg-slate-900/30 dark:text-slate-400 border-slate-200 dark:border-slate-800/50";
      default:
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 bg-popover border rounded-md shadow-lg z-20 w-32 py-1 animate-in fade-in-50 zoom-in-95 slide-in-from-top-2">
          {HTTP_METHODS.map((method) => (
            ...
            >
              <div
                className={`w-2 h-2 rounded-full mr-2 ${method === value ? "bg-primary" : "bg-transparent"}`}
              ></div>
              {method}
            </div>
          ))}
